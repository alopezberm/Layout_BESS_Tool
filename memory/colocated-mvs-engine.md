...
**Update 2026-06-24 — grid modes removed, Row Pack is primary.** The four grid heuristic modes (conservative/aggressive/ultra_aggressive/hyper_pack), `run_bess_optimization`, `MODE_PROFILES`, and the grid-only helpers in `placement.py` (`place_clusters`, `_score_mvs`, `_hyper_pack_pass`, `_try_add_mvs`, `_stragglers_pass_singlepass`) were deleted — the grid never achieved true back-to-back (min gap stuck at the lattice pitch ~1.5-2 m, never the 0.15 m back clearance). Replaced by **`core/packing.py` `run_row_packing`** (primary engine): analytic back-to-back shelf packing, adaptive per-row local search for concave polygons, all 4 rotations, interleaved centred MVS rows, cable-cap Hungarian. On the user's irregular site it serves ~42 BESS (manual was 40). Two alternatives tried and discarded: skyline 'brick' greedy (~31) and an exact CP-SAT/OR-Tools ILP (intractable: fine candidates -> ~5M conflicts, no solution in 60s; coarse can't reach 0.15 m). Ceiling is ~42 served / 44 physical under cable=25 m + 4 BESS/MVS; binding constraint is MVS stations displacing BESS. `placement.py` keeps `_prune_avail`, `_grow_cluster`, `_stragglers_pass`, `_optimal_reassign` (used by row_pack + co-located). `run_colocated_optimization` no longer takes `mode` (internal `_CO_PROFILE`). Renderers handle MultiPolygon sites.
